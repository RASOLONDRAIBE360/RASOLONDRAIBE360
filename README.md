...
<div align="center">
  <img src="https://raw.githubusercontent.com/RASOLONDRAIBE360/RASOLONDRAIBE360/main/banner.svg" alt="Stevy Bryan Philippe — Developer Banner" width="100%"/>
</div>
...
<section id="github-stats">
  ...
  <!-- Ligne du haut : 3 chiffres clés -->
  <div id="gh-top" class="gh-top-grid">
    <div class="gh-stat"><span class="spinner"></span></div>
    <div class="gh-stat"><span class="spinner"></span></div>
    <div class="gh-stat"><span class="spinner"></span></div>
  </div>

  <!-- Ligne du bas : langages + séries -->
  <div class="gh-bottom-grid">
    <div class="gh-card">
      <p class="gh-card-title">⟨/⟩ Langages principaux</p>
      <div id="gh-langs"><span class="spinner"></span></div>
    </div>
    <div class="gh-card">
      <p class="gh-card-title">🔥 Séries de contributions</p>
      <div id="gh-streak" class="gh-streak-grid"><span class="spinner"></span></div>
    </div>
  </div>

  <p id="gh-footer" class="gh-footer">Chargement des données GitHub…</p>
...
<script>
const USERNAME = 'RASOLONDRAIBE360';
...
async function fetchContributions() {
  try {
    const r = await fetch(`https://api.github.com/users/${USERNAME}/events?per_page=100`);
    const events = await r.json();
    if (!Array.isArray(events)) return {};

    const pushDays = new Set();
    events.forEach(e => {
      if (e.type === 'PushEvent') pushDays.add(e.created_at.slice(0, 10));
    });

    const days = Array.from(pushDays).sort().reverse();
    let streak = 0, best = 0, cur = 0;
    let streakEnd = '', bestStart = '', bestEnd = '', curStart = '';
    ...
    for (let i = 0; i < days.length; i++) {
      ...
      if (i === 0) {
        if (d === today || d === yesterday) {
          streak = 1; streakEnd = d; cur = 1; curStart = d;
        }
      } else {
        const diff = Math.round((new Date(days[i - 1]) - new Date(d)) / 86400000);
        if (diff === 1) {
          if (streak > 0) streak++;
          cur++; curStart = d;
        } else {
          if (cur > best) { best = cur; bestStart = curStart; bestEnd = days[i - 1]; }
          cur = 0;
        }
      }
    }
    if (cur > best) { best = cur; bestStart = curStart; bestEnd = days[0]; }

    const total = events
      .filter(e => e.type === 'PushEvent')
      .reduce((s, e) => s + (e.payload?.commits?.length || 0), 0);

    return { total, streak, best, streakEnd, bestStart, bestEnd };
  } catch { return {}; }
}
...
async function fetchUser() {
  try {
    ...
    return { since: fmtDate(u.created_at), repos: u.public_repos, followers: u.followers };
  } catch { return {}; }
}

async function initGitHubStats() {
  const [contrib, langs, user] = await Promise.all([
    fetchContributions(), fetchLangs(), fetchUser()
  ]);

  document.getElementById('gh-top').innerHTML = `
    <div class="gh-stat">
      <span class="gh-num" style="color:#7F77DD;">${contrib.total || '—'}</span>
      <span class="gh-lbl">Contributions récentes</span>
      <span class="gh-sub">90 derniers jours</span>
    </div>
    <div class="gh-stat">
      <span class="gh-num" style="color:#1D9E75;">${contrib.streak || 0}</span>
      <span class="gh-lbl">Série actuelle</span>
      <span class="gh-sub">${fmtDate(contrib.streakEnd)}</span>
    </div>
    <div class="gh-stat">
      <span class="gh-num" style="color:#378ADD;">${contrib.best || 0}</span>
      <span class="gh-lbl">Meilleure série</span>
      <span class="gh-sub">${contrib.bestStart
        ? fmtDate(contrib.bestStart) + ' – ' + fmtDate(contrib.bestEnd)
        : '—'}</span>
    </div>`;

  document.getElementById('gh-langs').innerHTML = langs.length
    ? langs.map(l => {
        const c = LANG_COLORS[l.name] || LANG_COLORS.default;
        return `
          <div class="gh-bar-row">
            <span class="gh-dot" style="background:${c};"></span>
            <span class="gh-bar-name">${l.name}</span>
            <div class="gh-bar-track">
              <div class="gh-bar-fill" style="width:${l.pct}%; background:${c};"></div>
            </div>
            <span class="gh-bar-pct">${l.pct}%</span>
          </div>`;
      }).join('')
    : '<span style="color:#484f58; font-size:12px;">Données indisponibles</span>';

  document.getElementById('gh-streak').innerHTML = `
    <div class="gh-streak-item">
      <span class="gh-streak-num" style="color:#7F77DD;">${contrib.streak || 0}</span>
      <span class="gh-streak-lbl">Série actuelle</span>
      <span class="gh-streak-date">${fmtDate(contrib.streakEnd)}</span>
    </div>
    <div class="gh-streak-item">
      <span class="gh-streak-num" style="color:#378ADD;">${contrib.best || 0}</span>
      <span class="gh-streak-lbl">Meilleure série</span>
      <span class="gh-streak-date">${fmtDate(contrib.bestStart)}</span>
    </div>
    <div class="gh-streak-item" style="margin-top:8px;">
      <span class="gh-streak-num" style="color:#e6edf3; font-size:18px;">${user.repos || '—'}</span>
      <span class="gh-streak-lbl">Dépôts publics</span>
      <span class="gh-streak-date">GitHub</span>
    </div>
    <div class="gh-streak-item" style="margin-top:8px;">
      <span class="gh-streak-num" style="color:#1D9E75; font-size:18px;">${user.followers || '—'}</span>
      <span class="gh-streak-lbl">Abonnés</span>
      <span class="gh-streak-date">Depuis ${user.since || '—'}</span>
    </div>`;

  document.getElementById('gh-footer').textContent =
    `↻ Données en direct · API GitHub · ${USERNAME}`;
}
